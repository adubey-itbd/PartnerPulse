...
pipeline (HaloPSA + TeamGPS + local docs → Azure gpt-5.4 churn analysis → JSON caches)
...
republishes Firestore. **Local dev is unchanged** — `server.py` + `data/*.json`, no auth
...
python scripts/seed_secrets.py               # load Halo/TeamGPS/Azure/Graph keys → Secret Manager
...
- **Cloud pipeline (NEW 2026-06-16, see `docs/Cloud-Pipeline-SOP.md`)** — the cycle
  ...
  **21:00 America/New_York**. Entrypoint `scripts/cloud_sync.py`; image from
  ...
  `Transcripts/` persisted in a GCS state bucket so the gpt-5.4 cache + transcript
  history survive between runs. **Builds are INCREMENTAL** — gpt-5.4 churn analysis
