...
Cloud Scheduler  ── daily 21:00 America/New_York ──▶  Cloud Run Job: partnerpulse-nightly
   ...
   3. extract.build_all                (Halo + TeamGPS + gpt-5.4, incremental)
...
keeps the gpt-5.4 cache (no score drift) and transcript history (Teams ~90-day
...
| Secrets | `halo-client-id`, `halo-client-secret`, `teamgps-api-key`, `azure-openai-key`, `graph-tenant-id`, `graph-client-id`, `graph-client-secret` |
...
gcloud run jobs create partnerpulse-nightly \
  ...
  --set-secrets="HALO_CLIENT_ID=halo-client-id:latest,HALO_CLIENT_SECRET=halo-client-secret:latest,TEAMGPS_API_KEY=teamgps-api-key:latest,AZURE_OPENAI_KEY=azure-openai-key:latest,GRAPH_TENANT_ID=graph-tenant-id:latest,GRAPH_CLIENT_ID=graph-client-id:latest,GRAPH_CLIENT_SECRET=graph-client-secret:latest"
...
- **Cost:** Run Job a few min/day + Scheduler (free tier) + tiny bucket ≈ pennies/mo;
  the gpt-5.4 calls are the real cost and are cached/incremental.
- **Outstanding follow-ups:** no **Azure budget alert** is set on the gpt-5.4
  resource — add a Cost Management budget so a full rebuild can't run up token cost
  unnoticed. Two dashboard-side hardening items are also open (tracked in
