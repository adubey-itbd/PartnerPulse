...
- **Transcript ingestion for every partner with a `Transcripts/` folder:** `scripts/build_real_partners.py` now parses local `.docx` transcripts (when `markitdown` is available) and feeds them to the AI, same as the registry path. Folder matching is case/punctuation-insensitive (`extract/transcripts.py: resolve_partner_dir`), and any `Transcripts/` subfolder that matches **no** built partner triggers a loud warning instead of being silently ignored.
