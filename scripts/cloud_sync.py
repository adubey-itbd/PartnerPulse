...
publishes the result to Cloud Firestore, all in one container:
  ...
  3. extract.build_all                 (Halo + TeamGPS + docs + Grok, incremental)
...
Why persist state: the pipeline is INCREMENTAL — the Grok churn analysis is
...
Env:
  ...
  pipeline secrets        HALO_*, TEAMGPS_*, AI_API_KEY/AI_BASE_URL/AI_MODEL,
                          GRAPH_* — injected from
...
# incremental Grok cache is effectively empty and we'd trigger a full cold
...
def main():
    if STATE_BUCKET:
        ...
        # incomplete; building now would cold-start the whole Grok cache.
        ...
        if data_n < DATA_FLOOR:
            raise SystemExit(
                ...
                f"trigger a full cold Grok re-run / score drift.")
    else:
